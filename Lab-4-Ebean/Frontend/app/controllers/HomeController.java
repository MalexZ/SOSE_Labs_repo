...
public class HomeController extends Controller {
    ...
    public CompletionStage<Result> outputMetadata() {
        ...
        if (titleBlank.hasErrors()) {
            return (CompletionStage<Result>) badRequest(views.html.paperTometadata_list.render(Arrays.asList("Error")));
        }
        ...
        return titleBlank.get().outputMetadata()
                .thenApplyAsync((WSResponse r) -> {
                    if (r != null) {
                        ...
                        return ok(views.html.paperTometadata_list.render(metadata));
                    } else {
                        System.out.println("response null");
    ...
    public CompletionStage<Result> findCategoryArticles() {

        Form<TitlesRequest> titleReqForm = formFactory.form(TitlesRequest.class).bindFromRequest();
        if (titleReqForm.hasErrors()) {
            return (CompletionStage<Result>) badRequest(views.html.category_titles.render(Arrays.asList("Error")));
        }

        return titleReqForm.get().findCategoryArticles()
                .thenApplyAsync((WSResponse r) -> {
                    if (r != null) {
                        System.out.println("findCategoryArticles response: " + r.asJson());
                        String allTitles = "";
                        List<String> titlesList = new ArrayList<>();

                        for (JsonNode node: r.asJson()) {
                            allTitles = node.get("titles").toString();
                            System.out.println(allTitles);
                        }
                        //String[] a = locations.toArray(new String[locations.size()]);

                        //Split String into one list.
                        titlesList.add(allTitles);
                        //["X||Y||Z"]
                        //Remove all empty elements.
                        //titlesList.removeAll(singleton(""));
                        //System.out.println(titlesList);

                        return ok(views.html.category_titles.render(titlesList));
                    } else {
                        System.out.println("response null");
                        String authorizeMessage = "Incorrect, error";
                        return badRequest(views.html.login.render(authorizeMessage));
    ...
    public CompletionStage<Result> WFController() {

        Form<FindBestWF> workflowForm = formFactory.form(FindBestWF.class).bindFromRequest();
        if (workflowForm.hasErrors()) {
            return (CompletionStage<Result>) badRequest(views.html.Best_workflow.render(Arrays.asList("Error")));
        }

        return workflowForm.get().outputWF()
                .thenApplyAsync((WSResponse r) -> {
                    if (r != null) {
                        System.out.println("findBestWorkflow response: " + r.asJson());
                        List<String> workflowList = new ArrayList<>();

                        for (JsonNode node: r.asJson()) {
                            workflowList.add(node.toString());
                        }

                        System.out.println(workflowList);

                        return ok(views.html.Best_workflow.render(workflowList));
                    } else {
                        System.out.println("response null");
                        String authorizeMessage = "Incorrect, error";
                        return badRequest(views.html.login.render(authorizeMessage));
    ...
    public CompletionStage<Result> AllWFController() {

        Form<FindAllWF> allWorkflowForm = formFactory.form(FindAllWF.class).bindFromRequest();
        if (allWorkflowForm.hasErrors()) {
            return (CompletionStage<Result>) badRequest(views.html.All_workflow.render(Arrays.asList("Error")));
        }

        return allWorkflowForm.get().outputAllWF()
                .thenApplyAsync((WSResponse r) -> {
                    if (r != null) {
                        System.out.println("findAllWorkflow response: " + r.asJson());
                        List<String> allWorkflowList = new ArrayList<>();

                        for (JsonNode node: r.asJson()) {
                            allWorkflowList.add(node.toString());
                        }

                        System.out.println(allWorkflowList);

                        return ok(views.html.All_workflow.render(allWorkflowList));
                    } else {
                        System.out.println("response null");
                        String authorizeMessage = "Incorrect, error";
                        return badRequest(views.html.login.render(authorizeMessage));
    ...
    public void displayBestWorkFlow(){
        MinimizingMakeChange mmc = new MinimizingMakeChange();
    }
